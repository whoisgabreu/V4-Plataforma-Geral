...
def buscar_projetos(url, email, status):
    """Busca projetos com tratamento de erro"""
    try:
        response = req.get(url, headers={"x-api-key": "4815162342"}, params={"email": email, "status": status}, timeout=10)
...
def home():
    ...
    ativos_data = buscar_projetos(
        "https://n8n.v4lisboatech.com.br/webhook/projetos",
   session["email"],
        "ativo"
    )
